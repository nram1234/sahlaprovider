    ...
    return Directionality(
      ...
      child: SafeArea(
        ...
        child: Scaffold(
          appBar: AppBar(
            centerTitle: true,
            title: Text('طلبات مؤجلة'),
          ),
          body: token != null
              ? StreamBuilder<Get_Future_Orders_json>(
                  stream: _allNetworking
                      .get_future_orders(token_id: token)
                      .asStream(),
                    ...
                    if (snapshot.hasData) {
                      if (snapshot.data.result.allOrders.length > 0) {
                        List<AllOrders> allOrders =
                            snapshot.data.result.allOrders;
                        for (AllOrders i in allOrders) {
                          DateTime dateTime = DateTime.parse(snapshot
                              .data.result.allOrders[0].timeExecutionRequest);
